  ...
  return (
    <div className="p-8 space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800/80 pb-6">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100 flex items-center gap-2.5">
              <Sliders className="w-6 h-6 text-sky-500 dark:text-sky-400" />
        ...
        <div className="flex items-center gap-3">
          <Button
            ...
            size="sm"
            onClick={fetchData}
            className="border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 text-xs gap-1.5"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="border-slate-200 dark:border-slate-800/80 p-5">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Empirically verified trajectory improvements</p>
        </Card>
        ...
        <Card className="border-slate-200 dark:border-slate-800/80 p-5">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Unnecessary or ineffective actuations</p>
        </Card>
        ...
        <Card className="border-slate-200 dark:border-slate-800/80 p-5">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Decision to device wire dispatch SLA</p>
        </Card>
        ...
        <Card className="border-slate-200 dark:border-slate-800/80 p-5">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Evaluated across fail-closed safety layer</p>
        </Card>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="outline" className="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/30 font-mono text-xs">
            {activeAutomations.length} Running
          </Badge>
        </div>

        {activeAutomations.length === 0 ? (
          <Card className="border-slate-200 dark:border-slate-800/60 p-8 text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {activeAutomations.map((item) => (
              <Card key={item.id} className="border-slate-200 dark:border-slate-800 p-5 space-y-4">
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">{item.policy?.name || 'Automation Action'}</h3>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{item.triggerReason}</p>
                  </div>
                  <Badge className="bg-sky-500/20 text-sky-600 dark:text-sky-400 border-sky-500/40 font-mono text-[10px] animate-pulse">
                    {item.status}
                  </Badge>
                </div>

                <div className="grid grid-cols-2 gap-3 text-xs font-mono bg-slate-50 dark:bg-slate-950/60 p-3 rounded border border-slate-200 dark:border-slate-800/60">
                  <div>
                    <span className="text-slate-500 block text-[10px]">ACTUATOR DEVICE</span>
                    <span className="text-slate-800 dark:text-slate-200 font-medium">{item.device?.name}</span>
                  </div>
                  <div>
                    <span className="text-slate-500 block text-[10px]">ROOM LOCATION</span>
                    <span className="text-slate-800 dark:text-slate-200">{item.device?.room?.name || 'Household'}</span>
                  </div>
                  <div>
                    <span className="text-slate-500 block text-[10px]">ACTION ISSUED</span>
                    <span className="text-emerald-600 dark:text-emerald-400 font-bold">{item.actionTaken}</span>
                  </div>
                  <div>
                    <span className="text-slate-500 block text-[10px]">VERIFICATION STATUS</span>
                    <span className="text-amber-600 dark:text-amber-400">{item.verificationStatus}</span>
                  </div>
                </div>

                <p className="text-xs text-slate-500 dark:text-slate-400 italic">Expected: {item.expectedOutcome}</p>
              </Card>
            ))}
          </div>
        )}
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {policies.map((p) => {
            ...
            return (
              <Card key={p.id} className="border-slate-200 dark:border-slate-800 p-5 space-y-4">
                <div className="flex items-start justify-between">
                ...
                <div className="flex items-center justify-between pt-2 border-t border-slate-200 dark:border-slate-800/60">
                  ...
                  <Button
                    ...
                    className="h-7 text-xs border-slate-300 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300"
                  >
                    Switch Mode
                  </Button>
                </div>

                <div className="grid grid-cols-3 gap-2 text-[11px] font-mono bg-slate-50 dark:bg-slate-950/40 p-2.5 rounded border border-slate-200 dark:border-slate-800/40 text-slate-600 dark:text-slate-400">
                  <div>
                    ...
                    return (
                      <React.Fragment key={exec.id}>
                        <tr className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                          ...
                          <td className="p-3.5">
                            <span className="font-semibold text-slate-900 dark:text-slate-200 block">{exec.policy?.name || 'Policy'}</span>
                            <span className="text-[11px] text-slate-500">{exec.triggerReason}</span>
                          </td>
                          <td className="p-3.5 whitespace-nowrap">
                            <span className="text-slate-800 dark:text-slate-200">{exec.device?.name}</span>
                            <span className="text-emerald-600 dark:text-emerald-400 font-bold block">{exec.actionTaken}</span>
                          </td>
                          <td className="p-3.5">
                            {exec.metricDelta != null ? (
                              <span className={isEffective ? 'text-emerald-600 dark:text-emerald-400 font-bold' : 'text-amber-600 dark:text-amber-400'}>
                            ...
                            <Badge
                              className={
                                isEffective
                                  ...
                                  ? 'bg-sky-500/10 text-sky-600 dark:text-sky-400 border-sky-500/30 text-[10px]'
                          ...
                          <td className="p-3.5 text-right">
                            <Button
                              size="sm"
                              variant="ghost"
                              className="h-7 text-xs text-sky-600 dark:text-sky-400 hover:text-sky-500 dark:hover:text-sky-300 gap-1"
                              onClick={() =>
                        ...
                        {isExpanded && (
                          <tr className="bg-slate-50 dark:bg-slate-950/90">
                            <td colSpan={6} className="p-4 border-t border-slate-200 dark:border-slate-800/80">
                              <div className="space-y-2 text-xs font-mono">
                                <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">
                                  DETERMINISTIC EXPLAINABILITY AUDIT PROOF (NO LLM)
                                </span>
                                <pre className="bg-white dark:bg-slate-900 p-3 rounded text-slate-800 dark:text-slate-300 border border-slate-200 dark:border-slate-800 whitespace-pre-wrap leading-relaxed">
                                  {exec.decisionExplanation}
                                </pre>
                                {exec.observedOutcome && (
                                  <p className="text-emerald-600 dark:text-emerald-400 text-xs">
                                    <span className="text-slate-600 dark:text-slate-400 font-bold">Observed Verification: </span>
                                    {exec.observedOutcome}
                                  </p>
                                )}
                              </div>
