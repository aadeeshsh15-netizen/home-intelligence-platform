    ...
    <div className="space-y-6 pb-12">
      ...
      <div>
        <h2 className="text-xs font-mono uppercase tracking-wider text-slate-400 mb-3 flex items-center gap-2">
          <Server className="w-3.5 h-3.5 text-sky-400" /> Deterministic Subsystem Health Checks
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {Object.entries(subsystems).map(([key, check]: [string, any]) => {
            ...
            return (
              <Card key={key} className="bg-slate-900/60 border-slate-800/80 p-3">
                ...
                <div className="mt-2 text-sm font-bold text-white font-mono flex items-center justify-between">
      ...
      <Card className="bg-slate-900/80 border-slate-800">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
            ...
            <div className="relative w-full md:w-64">
              ...
              <input
                ...
                className="w-full bg-slate-950 border border-slate-800 rounded pl-8 pr-3 py-1 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-sky-500 font-mono"
              />
            </div>
          </div>
        ...
        <CardContent className="p-0">
          {timelineData.events.length === 0 ? (
            ...
            <div className="divide-y divide-slate-800/80">
              {timelineData.events.map((evt: any) => {
                ...
                return (
                  <div key={evt.id} className="p-3 hover:bg-slate-950/40 transition-colors">
                    <div className="flex items-start justify-between gap-4">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-semibold text-xs text-slate-200">{evt.eventType}</span>
                        ...
                        <p className="text-xs text-slate-300 font-sans">{evt.summary}</p>
                      </div>
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        ...
                        {evt.metadata && (
                          <button
                            ...
                            className="text-[11px] font-mono text-sky-400 hover:text-sky-300 flex items-center gap-0.5"
                          >
                    ...
                    {isExpanded && evt.metadata && (
                      <div className="mt-2.5 p-2 bg-slate-950 rounded border border-slate-800">
          ...
          {/* Pagination Controls */}
          <div className="flex items-center justify-between p-3 border-t border-slate-800 font-mono text-xs text-slate-400">
            <div>
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                className="text-xs border-slate-800 bg-slate-950"
              >
                Previous
              </Button>
          ...
          <table className="w-full text-left text-xs font-mono">
            ...
            <tbody className="divide-y divide-slate-800">
              {devices.map((device) => {
                ...
                return (
                  <tr key={device.id} className="hover:bg-slate-950/40">
                    <td className="p-3 font-semibold text-slate-200">{device.name}</td>
                    <td className="p-3 text-slate-400">{device.identifier}</td>
                    <td className="p-3">
                      <Badge className="bg-slate-800 text-slate-300 text-[10px]">{device.protocol}</Badge>
                    </td>
